...
public class CadastroActivity extends AppCompatActivity {
    ...
    private void validaCamposCadastro() {
        //recupernado os valores do input que o usuario digitou
        String textoNome = campoNome.getText().toString();
        String textoEmail = campoEmail.getText().toString();
        String textoSenha = campoSenha.getText().toString();

        //Validar para ver se os campos foram preenchidos
    ...
    public void cadastrarUsuario(){
        //Recuperando a instancia do objeto do Firebase para autenticacao
        ...
        autenticacao.createUserWithEmailAndPassword(
          usuario.getEmail(), usuario.getSenha()
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if( task.isSuccessful()) { //usuario cadastrado com sucesso
                    String idUsuario = Base64Custom.codificarBase64(usuario.getEmail());
                    usuario.setIdUsuario(idUsuario);
                    usuario.salvar();
                    finish();
                } else {
                    //Tipos de erros que o firebase retorna caso a task não der success
                    String excecao = "";
                    try {
